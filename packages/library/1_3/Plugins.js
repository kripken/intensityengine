...
bakePlugins = function(_parent, plugins) {
    ...
    forEach(slots, function(slot) {
        var old = _parent.prototype[slot];

        eval(assert(' _parent.prototype[__MODULAR_PREFIX + _parent.prototype._class + slot] === undefined '));
        ...
        forEach(plugins, function(plugin) {
            var possible = plugin[slot];
            if (possible !== undefined && possible !== null) {
                callees.push(possible);
            }
        });
        if (callees.length === 0) return; // Nothing to do here
        if (callees.length === 1 && callees[0] === old) return; // It will already be called

        _classData[__MODULAR_PREFIX + _parent.prototype._class + slot] = callees;

        _classData[slot] = function() {
    ...
    forEach(plugins, function(plugin) {
        for (item in plugin) {
            if (typeof plugin[item] !== 'function' || findValue(slots, item) === -1) {
                _classData[item] = plugin[item];
            }
        }
    });

    return _parent.extend(_classData);
};
